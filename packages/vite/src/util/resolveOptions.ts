...
export function resolveOptions(options: CssUtilityOptions, modulesMap: ModulesMap, config: ResolvedConfig, generateScopedName?: (name: string, filePath: string, css: string) => string): ResolvedCssUtilityOptions {
  const { mode } = config;
